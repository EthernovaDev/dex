...
function useMockV1Pair(inputCurrency?: Currency): MockV1Pair | undefined {
  ...
  const wrapped = token ? WRAPPED_NATIVE[token.chainId] : undefined
  const isWETH = Boolean(token && wrapped && token.equals(wrapped))
  ...
  return useMemo(
    () =>
      token && tokenBalance && ETHBalance && inputCurrency && wrapped
        ? new MockV1Pair(ETHBalance.raw, tokenBalance)
        : undefined,
    [ETHBalance, inputCurrency, token, tokenBalance, wrapped]
  )
}
