...
const PinnedButton = styled.button<{ disabled?: boolean }>`
...
export function CurrencySearch({
  ...
  const showETH: boolean = useMemo(() => {
    ...
    return s === '' || s === 'n' || s === 'no' || s === 'nov' || s === 'nova'
  }, [searchQuery])
  ...
  const wnovaChecksummed = isAddress(wnovaAddress)
  const tonyChecksummed = isAddress(tonyAddress)

  const wnovaToken = useMemo(() => {
    if (!wnovaChecksummed) return undefined
    return new Token(
      fallbackChainId,
      wnovaChecksummed,
  ...
  }, [config, fallbackChainId, wnovaChecksummed])

  const tonyToken = useMemo(() => {
    if (!tonyChecksummed) return undefined
    return new Token(
      fallbackChainId,
      tonyChecksummed,
  ...
  }, [config, fallbackChainId, tonyChecksummed])
  ...
  const pinnedItems = useMemo(
    () => [
      {
        key: 'NATIVE',
        currency: ETHER as Currency,
        label: NATIVE_SYMBOL,
        addressTail: 'native',
        error: null as string | null
      },
      {
        key: wnovaChecksummed || 'WNOVA',
        currency: wnovaToken,
        label: 'WNOVA',
        addressTail: wnovaChecksummed ? wnovaChecksummed.slice(-6) : 'missing',
        error: wnovaChecksummed ? null : 'WNOVA address missing in config'
      },
      {
        key: tonyChecksummed || 'TONY',
        currency: tonyToken,
        label: 'TONY',
        addressTail: tonyChecksummed ? tonyChecksummed.slice(-6) : 'missing',
        error: tonyChecksummed ? null : 'TONY address missing in config'
      }
    ],
    [tonyChecksummed, tonyToken, wnovaChecksummed, wnovaToken]
  )
    ...
    <Column style={{ width: '100%', flex: '1 1' }}>
      <PaddedColumn gap="14px">
        ...
        <RowBetween>
          ...
          <QuestionHelper text="Quick picks for Ethernova tokens." />
        </RowBetween>
        <PinnedRow>
          {pinnedItems.map(item => (
            ...
            >
              ...
              <div>
                ...
                <Text fontSize={12} color={item.error ? '#FF6871' : theme.text3}>
                  {item.error ?? `…${item.addressTail}`}
                </Text>
              </div>
            </PinnedButton>
          ))}
