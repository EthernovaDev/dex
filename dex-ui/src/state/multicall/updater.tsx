...
): Promise<{ results: (string | null)[]; blockNumber: number }> {
  ...
  try {
    if (chainId === 77777 && multicallContract.tryAggregate) {
      try {
        ...
        return { results: mapped, blockNumber: latestBlockNumber }
      } catch (error) {
        console.debug('tryAggregate failed, falling back to aggregate', error)
      }
    }
  ...
  useEffect(() => {
    ...
    cancellations.current = {
      ...
      cancellations: chunkedCalls.map((chunk, index) => {
        const { cancel, promise } = retry(() => fetchChunk(multicallContract, chunk, latestBlockNumber, chainId, latestBlockNumber), {
          n: Infinity,
          minWait: 2500,
          maxWait: 3500
        })
