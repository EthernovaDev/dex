...
import { Currency, CurrencyAmount, ETHER, JSBI, Pair, Token, TokenAmount, Trade } from '@im33357/uniswap-v2-sdk'
...
const BAD_RECIPIENT_ADDRESSES: string[] = [...BAD_RECIPIENTS]
...
export function useDerivedSwapInfo(): {
  ...
  const fallbackTrade = useMemo(() => {
    if (!fallbackPair || !parsedAmount || !inputCurrency || !outputCurrency) return undefined
    try {
      if (isExactIn) {
        return Trade.bestTradeExactIn([fallbackPair], parsedAmount, outputCurrency, {
          maxHops: 1,
          maxNumResults: 1
        })[0]
      }
      return Trade.bestTradeExactOut([fallbackPair], inputCurrency, parsedAmount, {
        maxHops: 1,
        maxNumResults: 1
      })[0]
    } catch {
  ...
  }, [fallbackPair, parsedAmount, inputCurrency, outputCurrency, isExactIn])

  const v2Trade = (isExactIn ? bestTradeExactIn : bestTradeExactOut) ?? fallbackTrade
