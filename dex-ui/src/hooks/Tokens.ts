...
export function useAllTokens(): { [address: string]: Token } {
  ...
  return useMemo(() => {
    ...
    const tony = isAddress(config.tokens.TONY.address)
    if (tony) {
      tokenMap[tony] = new Token(
        activeChainId,
        tony,
    ...
    return userAddedTokens.reduce<{ [address: string]: Token }>((map, token) => {
      map[token.address] = token
      return map
    }, tokenMap)
  }, [chainId, config, userAddedTokens, allTokens])
}
