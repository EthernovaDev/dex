...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount } from '@im33357/uniswap-v2-sdk'
    ...
    <>
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && WRAPPED_NATIVE[chainId] && token.equals(WRAPPED_NATIVE[chainId]) ? 'WNOVA' : token.symbol}/NOVA
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && WRAPPED_NATIVE[chainId] && token.equals(WRAPPED_NATIVE[chainId]) ? 'WNOVA' : token.symbol}:
        </Text>
        <RowFixed>
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const v2SpotPrice =
    chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WRAPPED_NATIVE[chainId])) : undefined
  ...
  return (
    <AutoColumn gap="20px">
      ...
      {!isFirstLiquidityProvider && largePriceDifference ? (
        <YellowCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            It{"'"}s best to deposit liquidity into NovaDEX V2 at a price you believe is correct. If the V2 price seems
          ...
          <AutoColumn gap="8px">
            <RowBetween>
              ...
              <TYPE.black>
                {v1SpotPrice?.toSignificant(6)} {token.symbol}/NOVA
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v1SpotPrice?.invert()?.toSignificant(6)} NOVA/{token.symbol}
              </TYPE.black>
            </RowBetween>
            ...
            <RowBetween>
              ...
              <TYPE.black>
                {v2SpotPrice?.toSignificant(6)} {token.symbol}/NOVA
              </TYPE.black>
            </RowBetween>
            <RowBetween>
              <div />
              <TYPE.black>
                {v2SpotPrice?.invert()?.toSignificant(6)} NOVA/{token.symbol}
              </TYPE.black>
            </RowBetween>
      ...
      {isFirstLiquidityProvider && (
        <PinkCard>
          <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
            You are the first liquidity provider for this pair on NovaDEX V2. Your liquidity will be migrated at the
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your legacy V1 ${token.symbol}/NOVA liquidity will become NovaDEX V2 ${token.symbol}/NOVA liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `UNI-V1-${token.symbol}`, 'Legacy V1')
        : undefined,
    ...
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        <AutoRow style={{ alignItems: 'center', justifyContent: 'space-between' }} gap="8px">
          ...
          <div>
            <QuestionHelper text="Migrate your liquidity tokens from legacy V1 to NovaDEX V2." />
          </div>
        </AutoRow>

        {!account ? (
          <TYPE.largeHeader>You must connect an account.</TYPE.largeHeader>
        ) : validatedAddress && chainId && WRAPPED_NATIVE[chainId] && token?.equals(WRAPPED_NATIVE[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because NovaDEX V2 uses WNOVA under the hood, your legacy V1 WNOVA/NOVA liquidity cannot be migrated. You
