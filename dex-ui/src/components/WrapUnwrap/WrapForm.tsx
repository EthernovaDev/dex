...
export default function WrapForm() {
  ...
  const { novaBalance, refreshBalances, status, error: balanceError } = useBalances()
  ...
  const maxAmount = useMemo(() => {
    if (status !== 'ready') return BigNumber.from(0)
    const buffer = parseUnits('0.01', 18)
    return novaBalance.gt(buffer) ? novaBalance.sub(buffer) : BigNumber.from(0)
  }, [novaBalance, status])
  ...
  const inputError = useMemo(() => {
    ...
    if (!account) return 'Connect wallet to wrap.'
    ...
    if (parsedAmount.gt(novaBalance)) return `Insufficient ${NATIVE_SYMBOL} balance.`
    return null
  }, [account, novaBalance, parsedAmount, status, wnovaAddress, wrongNetwork])
  ...
  const handleWrap = async () => {
    ...
    try {
      ...
      const estimatedGas = await wnovaContract.estimateGas.deposit({ value: parsedAmount })
      const gasLimit = estimatedGas.mul(120).div(100)
      const overrides: Record<string, any> = { value: parsedAmount, gasLimit }
      ...
      const tx = await wnovaContract.deposit(overrides)
      setTxHash(tx.hash)
      addTransaction(tx, { summary: `Wrap ${amount} ${NATIVE_SYMBOL} to WNOVA` })
      await tx.wait(1)
      setConfirmed(true)
      setAmount('')
  ...
  const depositCalldata = useMemo(() => WNOVA_IFACE.encodeFunctionData('deposit'), [])

  return (
    <FormCard>
      <AutoColumn gap="sm">
        <RowBetween>
          <TYPE.main>Wrap {NATIVE_SYMBOL} to WNOVA</TYPE.main>
          <TYPE.main color={theme.text2}>
            ...
            {account
              ? status === 'wrong_network'
                ...
                : status === 'loading'
                ? 'Loading…'
                : `${formatBalance(novaBalance)} ${NATIVE_SYMBOL}`
              : '-'}
          </TYPE.main>
        </RowBetween>
        <AmountRow>
        ...
        {!account ? (
          <ButtonLight onClick={toggleWalletModal}>Connect wallet to wrap</ButtonLight>
        ) : wrongNetwork ? (
          <ButtonPrimary onClick={handleSwitch}>Switch to Ethernova</ButtonPrimary>
        ) : (
          <ButtonPrimary disabled={Boolean(inputError) || pending} onClick={handleWrap}>
            {pending ? 'Waiting for confirmation…' : `Wrap ${NATIVE_SYMBOL} → WNOVA`}
          </ButtonPrimary>
        )}
        ...
        {debugEnabled && (
          <DebugPanel>
            ...
            <DebugRow>
              ...
              <DebugValue>deposit()</DebugValue>
            </DebugRow>
            <DebugRow>
              <DebugLabel>Calldata</DebugLabel>
              <DebugValue>{depositCalldata}</DebugValue>
            </DebugRow>
            <DebugRow>
              <DebugLabel>Copy calldata</DebugLabel>
              <DebugButton
                onClick={async () => {
                  const ok = await copyToClipboard(depositCalldata)
