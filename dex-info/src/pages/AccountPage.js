      ...
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            ...
            <Link lineHeight={'145.23%'} href={`${explorerBase}/address/${account}`} target="_blank">
              {' '}
              {account?.slice(0, 42)}{' '}
            </Link>
          </TYPE.body>
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WNOVA') {
                        p.pair.token1.symbol = 'NOVA'
                      }
                      if (p.pair.token0.symbol === 'WNOVA') {
                        p.pair.token0.symbol = 'NOVA'
                      }
                      return (
